...
- In the `Project Settings > Modules` section, import a Gradle module for the `pxf/server` directory. You may see an error saying that there's
no JDK set for Gradle. Just cancel and retry. It goes away the second time.
- In the `Project Settings > Modules` section, import a Maven module for the `pxf/automation` directory. You may do this by selecting the `pxf/automation/pom.xml` file.
- Restart IntelliJ
