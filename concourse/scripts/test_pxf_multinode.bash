...
function run_multinode_smoke_test() {
	...
	time ssh "${HADOOP_SSH_OPTS[@]}" "${HADOOP_USER}@${HADOOP_HOSTNAME}" "
		...
		${HDFS} dfs -mkdir -p /tmp && mkdir -p /tmp/pxf_test && \
	...
	echo "Found $(${HDFS} dfs -ls /tmp/pxf_test | grep -c pxf_test) items in /tmp/pxf_test"
...
function open_ssh_tunnels() {
	...
	if [[ ! -d dataproc_env_files ]]; then
		...
		ssh -fNT -M -S /tmp/hadoop2181 -L 2181:hadoop:2181 "${HADOOP_USER}@${HADOOP_HOSTNAME}"
		ssh -S /tmp/hadoop2181 -O check "${HADOOP_USER}@${HADOOP_HOSTNAME}"
	fi
}
...
function close_ssh_tunnels() {
	...
	if [[ ! -d dataproc_env_files ]]; then
		ssh -S /tmp/hadoop2181 -O exit "${HADOOP_USER}@${HADOOP_HOSTNAME}"
	fi
}
...
function run_pxf_automation() {
	...
	sed -i "s/GPDB_IP/${gpdb_master}/g" "$multiNodesCluster"
