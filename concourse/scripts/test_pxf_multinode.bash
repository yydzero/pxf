...
function run_multinode_smoke_test() {
	...
	time ssh "${HADOOP_SSH_OPTS[@]}" "${HADOOP_USER}@${HADOOP_HOSTNAME}" "
		...
		${HDFS_BIN}/hdfs dfs -chown -R gpadmin:gpadmin /tmp/pxf_test
	"

	echo "Found $("${HDFS_BIN}/hdfs" dfs -ls /tmp/pxf_test | grep -c pxf_test) items in /tmp/pxf_test"
...
function run_pxf_automation() {
	...
	su --login -c ~gpadmin/run_pxf_automation_test.sh gpadmin
}
