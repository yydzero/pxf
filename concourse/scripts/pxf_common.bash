...
function run_regression_test() {
	...
	su gpadmin -c "~gpadmin/run_regression_test.sh ${PWD}"
}

function build_install_gpdb() {
    pushd gpdb_src || return 1
...
function install_pxf_client() {
	...
	if [[ ${TEST_ENV} == dev ]]; then
		...
		pushd "${PXF_EXTENSIONS_DIR}" > /dev/null || return 1
...
function init_and_configure_pxf_server() {
	pushd "${PXF_HOME}" > /dev/null || return 1
