...
function readable_external_table_text_query() {
	local name=${1} path=${2} delimiter=${3:-'|'}
...
function read_and_validate_table_count() {
	local table_name="$1" expected_count="$2" num_rows
	num_rows=$(time psql -t -c "SELECT COUNT(l_linenumber) FROM $table_name" | tr -d ' ')
...
function sync_configuration() {
	gpssh -u gpadmin -h mdw -v -s -e "${GPHOME}/pxf/bin/pxf cluster sync"
}
...
function validate_write_to_gpdb() {
	local external=${1} internal=${2} external_values gpdb_values
...
function validate_write_to_external() {
	local benchmark_name=${1} path=${2} name=${benchmark_name}_from_write external_values
	...
	external_values=$(psql -t -c "SELECT ${VALIDATION_QUERY} FROM lineitem_${name}_read")
	write_sub_header 'Results from external query'
	echo "${external_values}"
...
function configure_adl_server() {
	...
	gpssh -u gpadmin -h mdw -v -s -e "
		mkdir -p $ADL_SERVER_DIR &&
		sed -e 's|YOUR_ADL_REFRESH_URL|${ADL_REFRESH_URL}|' \
		    -e 's|YOUR_ADL_CLIENT_ID|${ADL_CLIENT_ID}|' \
		    -e 's|YOUR_ADL_CREDENTIAL|${ADL_CREDENTIAL}|' ${PXF_CONF_DIR}/templates/adl-site.xml > ${ADL_SERVER_DIR}/adl-site.xml
	"
	sync_configuration
}
...
function configure_gcs_server() {
	...
	gpssh -u gpadmin -h mdw -v -s -e "
		mkdir -p $GS_SERVER_DIR &&
		sed -e 's|YOUR_GOOGLE_STORAGE_KEYFILE|${GS_SERVER_DIR}/gsc-ci-service-account.key.json|' ${PXF_CONF_DIR}/templates/gs-site.xml > ${GS_SERVER_DIR}/gs-site.xml
	"
...
function configure_s3_extension() {
	...
	EOF
	grep < cluster_env_files/etc_hostfile sdw | cut -d ' ' -f 2 > /tmp/segment_hosts
...
function configure_s3_server() {
	...
	gpssh -u gpadmin -h mdw -v -s -e "mkdir -p $S3_SERVER_DIR &&
		sed -e 's|YOUR_AWS_ACCESS_KEY_ID|${AWS_ACCESS_KEY_ID}|' \
	  	    -e 's|YOUR_AWS_SECRET_ACCESS_KEY|${AWS_SECRET_ACCESS_KEY}|' ${PXF_CONF_DIR}/templates/s3-site.xml > ${S3_SERVER_DIR}/s3-site.xml"
	sync_configuration
}
...
function configure_wasb_server() {
	...
	gpssh -u gpadmin -h mdw -v -s -e "mkdir -p $WASB_SERVER_DIR &&
		sed -e 's|YOUR_AZURE_BLOB_STORAGE_ACCOUNT_NAME|${WASB_ACCOUNT_NAME}|' \
		    -e 's|YOUR_AZURE_BLOB_STORAGE_ACCOUNT_KEY|${WASB_ACCOUNT_KEY}|' ${PXF_CONF_DIR}/templates/wasbs-site.xml > ${WASB_SERVER_DIR}/wasbs-site.xml"
	sync_configuration
}
...
function run_concurrent_benchmark() {
	local benchmark_fn=${1} prepare_test_fn=${2} benchmark_name=${3} benchmark_description=${4} concurrency=${5}
	local pids=() status_codes=() has_failures
	...
	find . -name "/tmp/${benchmark_fn}-${benchmark_name}-*.bench" -print0 | xargs -0 cat
	...
	if (( has_failures > 0 )); then
		echo "Had ${has_failures} failures"
		exit 1
	fi
}

function run_text_benchmark() {
	local prepare_test_fn=${1} benchmark_name=${2} benchmark_description=${3} run_id=${4}
	local name=${benchmark_name}_${run_id} write_count
	...
	write_count=$(time psql -c "INSERT INTO lineitem_${name}_write SELECT * FROM lineitem" | awk '{print $3}')
...
function run_parquet_benchmark() {
	local prepare_test_fn=${1} benchmark_name=${2} benchmark_description=${3} run_id=${4}
	local name=${benchmark_name}_${run_id} write_parquet_count

	"${prepare_test_fn}" "${name}" "${run_id}"

	write_header "${benchmark_description} WRITE PARQUET BENCHMARK (Run ${run_id})"
	write_parquet_count=$(time psql -c "INSERT INTO lineitem_${name}_write_parquet SELECT * FROM lineitem" | awk '{print $3}')
...
function main() {
	...
	if [[ ${BENCHMARK_S3} == true ]]; then
		...
		if (( concurrency == 1 )); then
			...
			run_concurrent_benchmark run_parquet_benchmark create_s3_parquet_tabes s3 S3 "${concurrency}"
		fi
	fi
