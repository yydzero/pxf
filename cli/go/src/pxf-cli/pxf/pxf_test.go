...
var _ = Describe("CommandFunc", func() {
	...
	It("Is successful when GPHOME and PXF_CONF are set and init is called", func() {
		commandFunc, err := pxf.CommandFunc(pxf.Init, globalCluster)
		Expect(err).To(BeNil())
	...
	It("Is successful when GPHOME is set and start/stop are called", func() {
		commandFunc, err := pxf.CommandFunc(pxf.Start, globalCluster)
		Expect(err).To(BeNil())
		Expect(commandFunc(1)).To(Equal("/test/gphome/pxf/bin/pxf start"))
		commandFunc, err = pxf.CommandFunc(pxf.Stop, globalCluster)
		Expect(err).To(BeNil())
	...
	It("Fails to init, start, sync, or stop when GPHOME is not set", func() {
		...
		Expect(commandFunc).To(BeNil())
		Expect(err).To(Equal(errors.New("GPHOME must be set")))
		commandFunc, err = pxf.CommandFunc(pxf.Sync, globalCluster)
		Expect(commandFunc).To(BeNil())
		Expect(err).To(Equal(errors.New("GPHOME must be set")))
		commandFunc, err = pxf.CommandFunc(pxf.Stop, globalCluster)
		Expect(commandFunc).To(BeNil())
